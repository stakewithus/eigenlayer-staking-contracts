...
contract EigenUserTest is Test, IEigenUserEvents, WithdrawalCredentialsProof {
    ...
    function test_receive(uint256 amount_) public {
        ...
        vm.expectEmit();
        emit ExecutionRewards(toUser, toTreasury);
        (bool sent, ) = address(eigenUser).call{value: amount_}("");
        assertTrue(sent);
    ...
    function test_queueRestakingRewards(uint256 amount_) public {
        ...
        assertEq(eigenUser.getClaimableRewards(), 0);
        vm.roll(block.number + 21_001);
        assertEq(eigenUser.getClaimableRewards(), toUser);
